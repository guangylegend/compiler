				...
				if(child.get(i) instanceof relational_expression)
				{
					...
					if(((returnrecord)child.get(i).record).constant)
					{
						if(r.constant)
						{
							...
							{
								if(child.get(i-1).record.equals("=="))r.value = (r.value.equals(((returnrecord)child.get(i).record).value));
								else r.value = (!r.value.equals(((returnrecord)child.get(i).record).value));
							}
						}
						else
						{
