...
public class inclusive_or_expression extends root
{
	...
	public int check() throws Exception
	{
		...
		else
		{
			...
			{
				if(child.get(i) instanceof exclusive_or_expression)
				{
					...
					if(((returnrecord)child.get(i).record).constant)
					{
						if(r.constant)
						{
							...
							else r.value = ((int)r.value | (int)((returnrecord)child.get(i).record).value);
						}
						else
						{
